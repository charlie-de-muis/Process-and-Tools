...
# Configuration
DATA_DIR = "./data"  # Directory containing the JSON files
DB_FILE = "Cargohub_db.sqlite"  # SQLite database file
...
def insert_json_data(json_file, table_name):
    ...
    # Check if the data is in the form of a list (array of records)
    if isinstance(data, dict):
        data = [data]  # Wrap the single object in a list

    all_records_uploaded = True  # Flag to track if all records were successfully uploaded
    ...
    for record in data:
        ...
        for key, value in record.items():
            if isinstance(value, list):
                record[key] = json.dumps(value)  # Convert list to JSON string
            elif isinstance(value, dict):
                record[key] = json.dumps(value)  # Convert dict to JSON string

        # Replace missing values with default values dynamically
        values = []
        for key in record.keys():
            # If the value is None (missing), use a default value
            value = record.get(key, get_default_value(key))
            values.append(value)
    ...
    # If all records were uploaded successfully, print a success message
